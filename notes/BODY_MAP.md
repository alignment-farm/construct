...
| Operational field-use capture | **Cold-reviewed; armed at zero eligible invocations; one completed shadow trace adjudicated `capture_confounded`; clock not started** | [protocol](FIELD_USE_PROTOCOL_V0.md), [implementation review](FIELD_USE_CAPTURE_V0_REVIEW.md), [adjudication](FIELD_USE_INVOCATION_001_ADJUDICATION.md), [walkthrough](walkthrough/22_FIELD_USE_PILOT.md); `make body-field-use-test` | Independently necessary repository work only; shadow/consulted/negative use and lineage origin frozen before action; wire trace, never behavioral evidence |
...
The active foundation is **Body Core v0.3** engineering: a structural lineage
kernel and an explicitly selected lifecycle/placement/warrant policy projector. X2
...
adapter, or replay optimization was added. Its exact implementation is
[cold-endorsed](BODY_CORE_EXPLICIT_PROJECTION_REVIEW.md).

The current operational build is the manual
[field-use capture layer](FIELD_USE_CAPTURE_V0.md) under the separately
...
[cold-endorsed](FIELD_USE_CAPTURE_V0_REVIEW.md). The first attempted local
shadow trace completed structurally but was
...
invocation zero. The trace remains append-only but is excluded from the
eligible count. The pilot remains armed at zero; its fourteen-day clock has not
started.
